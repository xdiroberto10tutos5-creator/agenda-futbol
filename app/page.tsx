...
export default function AgendaFutbolMVP() {
  ...
  async function fetchClassRecords() {
    ...
    setExceptions(
      (data || []).map((record) => ({
        ...
        newDate:
          record.class_date !== record.original_date ? record.class_date : undefined,
  ...
  const generatedSessions = useMemo(() => {
    ...
    const endDate =
      endDateFromSelectedMonth > endDateFromCurrentWeek
        ? endDateFromSelectedMonth
        : endDateFromCurrentWeek;
  ...
  const sessionsWithCycles = useMemo(() => {
    ...
    students.forEach((student) => {
      const studentSessions = generatedSessions
        ...
        .sort((a, b) =>
          `${a.originalDate} ${a.time}`.localeCompare(
            `${b.originalDate} ${b.time}`
          )
        );
      ...
      const cycleSessions = sessionsWithCycles
        .filter(
          (session) =>
            ...
            session.cycleNumber === selectedCycleNumber
        )
        .sort((a, b) =>
          `${a.originalDate} ${a.time}`.localeCompare(
            `${b.originalDate} ${b.time}`
      ...
      const paid = payments
        .filter(
          (payment) =>
            ...
            payment.cycleNumber === selectedCycleNumber
        )
  ...
  const pendingMonthSessions = useMemo(() => {
    return visibleSessions.filter(
      (session) => getSessionStatus(session) === "programada"
    );
  }, [visibleSessions]);

  const reviewedMonthSessions = useMemo(() => {
    return visibleSessions.filter(
      (session) => getSessionStatus(session) !== "programada"
    );
  }, [visibleSessions]);
  ...
  const groupedSessions = useMemo(() => {
    ...
    pendingMonthSessions.forEach((session) => {
  ...
  const weeklySchedule = useMemo(() => {
    return sessionsWithCycles
      .filter((session) => {
        const originalIsThisWeek =
          session.originalDate >= currentWeek.start &&
          session.originalDate <= currentWeek.end;
        const currentDateIsThisWeek =
          session.date >= currentWeek.start && session.date <= currentWeek.end;
      ...
      .sort((a, b) =>
        `${a.weekDisplayDate} ${a.time}`.localeCompare(
          `${b.weekDisplayDate} ${b.time}`
        )
      );
  ...
  function toggleWeekday(dayId: number) {
    ...
    setStudentForm({
      ...studentForm,
      schedule: [
        ...studentForm.schedule,
        { weekday: dayId, time: "", place: "Cancha principal" },
      ],
    });
  }
        ...
        <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
          <div>
            ...
            <p className="text-slate-600">
              Sistema de clases, ciclos, asistencias y pagos.
            </p>
            ...
            <CardContent className="space-y-4 p-5">
              <div className="flex items-center gap-2">
                ...
                <h2 className="text-xl font-semibold">
                  {editingStudentId ? "Editar niño" : "Registrar niño"}
                </h2>
              </div>
              ...
              <div className="space-y-3">
                ...
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setStudentForm({ ...studentForm, startDate: e.target.value })
                    }
                  />
                </div>
                    ...
                    {diasSemana.map((day) => (
                      <button
                        ...
                        className={`rounded-full border px-3 py-2 text-sm ${
                          studentForm.schedule.some(
                            (item: any) => item.weekday === day.id
                          )
                      ...
                      >
                        ...
                        <div>
                          ...
                          <Input
                            ...
                            value={item.time}
                            onChange={(e) =>
                              updateScheduleItem(item.weekday, "time", e.target.value)
                            }
                          />
                        </div>
                        <div>
                          ...
                          <Input
                            value={item.place}
                            onChange={(e) =>
                              updateScheduleItem(item.weekday, "place", e.target.value)
                            }
            ...
            <CardContent className="space-y-4 p-5">
              ...
              <div className="overflow-x-auto rounded-xl border bg-white">
                ...
                <div className="grid min-w-[900px] grid-cols-7">
                  {weekDaysWithDates.map((day) => {
                    const daySessions = weeklyGroups.filter(
                    ...
                    return (
                      <div
                        key={day.date}
                        className="min-h-[260px] border-r p-3 last:border-r-0"
                      >
                        {daySessions.length === 0 ? (
                          ...
                          <div className="space-y-3">
                            {daySessions.map((item: any) => (
                              <div
                                key={`${item.date}-${item.time}-${item.place}`}
                                className="rounded-xl border bg-slate-50 p-3 shadow-sm"
                              >
                                <p className="text-sm font-semibold text-slate-500">
                                  {getDayName(createLocalDate(item.date).getDay())}
                                </p>
                                  ...
                                  {item.students.map((session: any) => {
                                    ...
                                    return (
                                      ...
                                      >
                                        ...
                                        {session.status === "pospuesta" && (
                                          <p className="text-[11px] text-slate-500">
                                            Nueva fecha: {session.date}
                                          </p>
                                        )}
                                      </div>
                                    );
              ...
              <div className="space-y-3">
                <h3 className="font-semibold">
                  Clases pendientes del mes seleccionado
                </h3>
                <p className="text-sm text-slate-600">
                  Aquí solo aparecen las clases programadas. Cuando marques Asistió,
                  No asistió o Posponer, se moverán al historial de abajo.
                </p>

                {groupedSessions.length === 0 && (
                  <p className="text-sm text-slate-500">
                    No hay clases pendientes en este mes.
                  </p>
                )}

                {groupedSessions.map((group: any) => (
                  <div
                    key={`${group.date}-${group.time}-${group.place}`}
                    className="rounded-xl border bg-white p-4"
                  >
                    <div>
                      <p className="text-sm font-semibold text-slate-500">
                        {getDayName(createLocalDate(group.date).getDay())}
                      </p>
                      <p className="font-semibold">
                        ...
                        >
                          <div>
                            ...
                            <p className="text-xs text-slate-500">
                              Ciclo {session.cycleNumber} - Fecha {session.classNumberInCycle}/
                              {getClassesPerCycle(
                                students.find(
                            ...
                            >
                              Estado: {getStatusText(getSessionStatus(session))}
                            </p>
                          </div>
                          ...
                          <div className="flex flex-wrap gap-2">
                            <Button
                              size="sm"
                              variant="outline"
                              onClick={() => setClassStatus(session, "asistio")}
                            >
                              <CheckCircle className="mr-1 h-4 w-4" /> Asistió
                            </Button>
                            <Button
                              size="sm"
                              variant="outline"
                              onClick={() => postponeClass(session)}
                            >
                              <Clock className="mr-1 h-4 w-4" /> Posponer
                            </Button>
                            <Button
                              size="sm"
                              variant="outline"
                              onClick={() => setClassStatus(session, "no_asistio")}
                            >
                              <XCircle className="mr-1 h-4 w-4" /> No asistió
                            </Button>
                          </div>
                        </div>
                      ))}
              ...
              <div className="space-y-3">
                <h3 className="font-semibold">Historial del mes seleccionado</h3>
                <p className="text-sm text-slate-600">
                  Aquí quedan las clases ya revisadas. Puedes restaurarlas si te
                  equivocaste.
                </p>

                {reviewedMonthSessions.length === 0 && (
                  <p className="text-sm text-slate-500">No hay clases revisadas.</p>
                )}

                {reviewedMonthSessions.map((session: any) => (
                  <div
                    key={`reviewed-${session.id}`}
                    className="flex flex-wrap items-center justify-between gap-2 rounded-xl border bg-white p-4"
                  >
                    <div>
                      <p className="text-sm font-semibold text-slate-500">
                        {getDayName(createLocalDate(session.date).getDay())}
                      </p>
                      <p className="font-semibold">
                        {session.date} - {session.time}
                      </p>
                      <p className="text-sm text-slate-600">{session.place}</p>
                      <p className="font-medium">{session.studentName}</p>
                      <p
                        className={`text-xs font-semibold ${getStatusColor(
                          getSessionStatus(session)
            ...
            <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
              <div>
                <div className="flex items-center gap-2">
                  ...
                  <h2 className="text-xl font-semibold">
                    Resumen de mensualidad por ciclo
                  </h2>
                </div>
                <p className="text-sm text-slate-600">
                  Cada ciclo dura 4 semanas. Si entrena 1 día por semana son 4
                  clases = $20; si entrena 2 días por semana son 8 clases = $40.
                </p>
              </div>
              ...
              <div className="w-full md:w-44">
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setSelectedCycleNumber(Number(e.target.value) || 1)
                  }
                />
              </div>
            </div>
                ...
                <div key={student.id} className="rounded-xl border bg-white p-4">
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      ...
                      <p className="text-sm text-slate-600">
                        Ciclo: {student.cycleNumber}
                      </p>
                      <p className="text-sm text-slate-600">
                        Periodo: {student.cycleStart} hasta {student.cycleEnd}
                      </p>
                      <p className="text-sm text-slate-600">
                        Clases cobradas: {student.classCount}/
                        {student.classesPerCycle}
                      </p>
                      ...
                      >
                        Estado: {student.status} {student.balance > 0 ? `$${student.balance}` : ""}
                      </p>
                    </div>
                    <div className="flex flex-col gap-2">
                      ...
                      <Button
                        ...
                        variant="outline"
                        onClick={() =>
                          navigator.clipboard.writeText(getWhatsappMessage(student))
                        }
                      >
                        Copiar WhatsApp
                      </Button>
